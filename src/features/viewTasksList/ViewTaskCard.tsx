...
const ViewTaskCard: React.FC<Props> = (props) => {
	...
	const renderTaskList = (taskList: any) => {
		return taskList.map((task: Task, index: number) => (
			<SectionRow className={"w-full h-10 justify-center items-end border-b"}>
				...
				<SectionRow className={"w-1/12 justify-end"}>{task.status}</SectionRow>
			</SectionRow>
		));
	};
	...
	return (
		<div
			className={"w-full mx-auto text-gray-900 bg-gray-50 rounded-2xl overflow-hidden "}
			style={{ height: "70vh" }}>
			<SectionRow className={"h-12 bg-amber-300 rounded-t-2xl pt-3 pl-4 font-bold "}>
				Tasks List
			</SectionRow>
			<SectionColumn className={"w-full  pt-5 px-5 "} style={{ height: "100%" }}>
				<SectionRow className={"w-full h-12 justify-center items-center border-b font-extrabold"}>
				...
				<div className=' mb-16 overflow-auto ' style={{ height: "auto" }}>
